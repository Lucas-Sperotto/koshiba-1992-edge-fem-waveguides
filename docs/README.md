...
A fase documental está fechada como baseline científica e didática. A implementação já inclui materiais por região, solver `k0 -> beta`, solver direto `beta -> k0/f`, exemplos Gmsh e pipeline de CSV/plots para as Figuras 3, 5 e 7. A validação quantitativa contra referência externa segue pendente e não deve ser declarada concluída sem CSV de referência, gráfico comparativo e erro registrado.

As imagens originais do artigo não são versionadas. Os esquemas em `docs/img/` são desenhos próprios do projeto, e os gráficos comparativos devem ser gerados em `out/validation/`.
